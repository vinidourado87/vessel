...
class VesselRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
  ...
  private val vesselTable = TableQuery[VesselTable]

  def createOrUpdate(vessel: Vessel) {
    val query = this.vesselTable.insertOrUpdate(vessel)
    db.run(query)
  }
  ...
  def relocate(id: Long, newLatitude: Double, newLongitude: Double) = {
    val query = vesselTable.filter(_.id === id).map(v => (v.latitude, v.longitude)).update((newLatitude, newLongitude))
    db.run(query)
    find(id)
  }

  def find(id: Long) = {
    val query = vesselTable.filter(_.id === id).result.headOption
    db.run(query)
  }

  def delete(id: Long) {
    val query = vesselTable.filter(_.id === id)
    db.run(query.delete)
  }
}
